...
const fs = require('fs');
const { toJSON, parseSeq, parseMap } = require('yaml/util');
...
class LTOHelper {
  ...
  async addIdentityEvent(chain, account, signingAccount) {
    if (!account.id) {
      throw new Error('Account doesn\'t have an id');
    }
  ...
  addResponseEvent(chain, account, processId, actionKey, data, key = 'ok') {
  ...
  async loadProcess(account, processId) {
